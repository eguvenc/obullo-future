    ...
    Bonsai.prototype = {
        ...
        setExpanded: function(listItem, expanded) {
            ...
            if (expanded) {
                ...
                listItem = $(listItem).addClass('collapsed').removeClass('expanded');
        ...
        insertCheckbox: function(listItem) {
            ...
            var id = this.generateId(listItem),
                checkbox = $('<input type="checkbox" name="' + this.getCheckboxName(listItem) + '" id="' + id + '" /> '),
            ...
            children.remove();
            listItem.append(checkbox).append(
                $('<label for="' + id + '">').append(text ? text : children.first())).append(text ? children : children.slice(1));
        },
        handleDuplicates: function() {
            ...
            self.el.on('change', 'input[type=checkbox]', function(ev) {
                ...
                var selector = 'input[type=checkbox]' + '[value="' + checkbox.val() + '"]' + '[name="' + checkbox.attr('name') + '"]' + (checkbox.prop('checked') ? ':not(:checked)' : ':checked');
        ...
        getCheckboxName: function(listItem) {
            return listItem.data('name') || listItem.parents().filter('[data-name]').data('name');
        },
        addExpandAllLink: function() {
            var self = this;
            $('<div class="expand-all">').append($('<a class="all">Expand all</a>').on('click', function() {
                self.expandAll();
            })).append('<i class="separator"></i>').append($('<a class="none">Collapse all</a>').on('click', function() {
                self.collapseAll();
        ...
        addSelectAllLink: function() {
            ...
            function getCheckboxes() {
                ...
                return scope.find('li').filter(self.options.selectAllExclude ||
                    function() {
                        return $(this).css('display') != 'none';
                    }).find('> input[type=checkbox]');
            }
            $('<div class="check-all">').append($('<a class="all">Select all</a>').css('cursor', 'pointer').on('click', function() {
                getCheckboxes().prop({
                    checked: true,
                    indeterminate: false
                });
            })).append('<i class="separator"></i>').append($('<a class="none">Select none</a>').css('cursor', 'pointer').on('click', function() {
                getCheckboxes().prop({
                    checked: false,
                    indeterminate: false
                });
        ...
        setCheckedValues: function(values) {
            ...
            $.each(values, function(key, value) {
                all.filter('[value="' + value + '"]').prop('checked', true).trigger('change');
            });
        }
    };
    $.extend(Bonsai, {
