...
var FORM_FACTORY = new function (){
    var forms = {};
    return {
        create : function(formId, showErrors, liveValidator, options) {
            
            if (! forms.formId) {

                forms[formId] = new FORM(formId, showErrors, liveValidator, options);

            }

            return forms[formId];
        },
        createTemplate : function (containerId, jsonData) { /* this function builds form template */
            ...
            try {
                obj.jsonData = jQuery.parseJSON(jsonData);
            } catch(error) {
                obj.jsonData = jsonData;
            }

            TEMPLATE.parse('templates/form.tpl', {formData:obj.jsonData}, containerId);
            
            /* get form info. Important */
            obj.formId = obj.jsonData.formId;

            return this.create(obj.formId, true);
 ...
 * @type Object
 */
var FORM = function(formId, showErrors, liveValidator, options){
    var obj = {
        formId                 : formId,
        ...
        ajax_options           : {async : true, type : 'post'}
    };
    ...
    var changeContainer = function() {
        ...
        if((! obj.containerId) || obj.containerElement.prop("tagName") == 'BODY') {
            ...
            if ( ! obj.containerId) {
                COMMON.logger.log('Auto generated container id for form : '+ obj.formId);
    ...
    obj.construct = function(){
        ...
        if (options) {
            obj.ajax_options.async = (options.async == false || options.async == true) ? options.async : obj.ajax_options.async;
            obj.ajax_options.type = (options.type) ? options.type : obj.ajax_options.type;
            obj.ajax_options.crossDomain = (options.crossDomain) ? options.crossDomain : false;
            obj.ajax_options.beforeSend =  (!COMMON.isEmpty(options) && options.beforeSend) ? options.beforeSend : null;
        }
        console.info(obj.ajax_options);
    ...
    obj.setError = function(input, msg, ref){ /* setting input's error message */
        var inputObj = getInputObj(input);
    ...
    obj.matches = function(second, first){
        ...
        var secondObj = getInputObj(second);
        var firstObj = getInputObj(first);
    ...
    obj.isValid = function(){
        ...
        var first = obj.formElement.parent('*').find('[data-error-for]:first').data('input_ref');
            ...
            if(str_split.length > 0)
            {
                for(var i in str_split)
                {
                    if (pass == true) {
                        ...
                        if(/^min\s?\(/i.test(str_split[i]))
                        {
                            var res = str_split[i].match( /min\((.*?)\)/i );
                        ...
                        if(/^max\s?\(/i.test(str_split[i]))
                        {
                            var res = str_split[i].match( /max\((.*?)\)/i );
                        ...
                        if(/^exact\s?\(/i.test(str_split[i]))
                        {
                            var res = str_split[i].match( /exact\((.*?)\)/i );
    ...
    obj.ajaxPost = function(closure){
        ...
        var formArgs = obj.formElement.formToJson();
        formArgs = (typeof obj.closureArgs == 'function') ? (obj.closureArgs(formArgs) || formArgs) : formArgs;

        COMMON.JQueryAjax(obj.formElement.attr('action'), formArgs, obj.ajax_options, function (res) {
    ...
    obj.checkAjax = function () { /* check if ajax-post or normal-post */
        if(obj.formElement.data('ajax') == 1)
            return true;
        COMMON.logger.warn('This form wont be sent as ajax request. Please make sure of ajax attribute must be "data-ajax=\'1\'" ') /* for debugging */

        return false;
    };

    obj.whenSubmit = function(closureAfter, closureBefore, closureArgs){
        
        obj.closureAfter  = closureAfter;
        obj.closureBefore = closureBefore;
        obj.closureArgs   = closureArgs;

        if(obj.formId == '') {
            COMMON.logger.error('There is no form to submit, please make sure of form-id');
            alert('FormJS : Error, review the console.');
        }
        ...
        $(document).off('submit', "#"+obj.formId).on('submit', "#"+obj.formId, function(){
            ...
            var execute = (typeof obj.closureBefore === 'function') ? (obj.closureBefore() || false) : true; /* beforeClosure return true or false; */

            COMMON.logger.log(obj.formId + ' -> submitting || Container id -> '+ obj.containerId);
            COMMON.logger.log('is vlaid : ' + isValid, ', Ajax request cannot be proceed, execute : ', execute);
            ...
            if( isValid && execute )
            {
                ...
                if(obj.checkAjax())
                {
                    ...
                    COMMON.logger.log('sending xmlhttp-request ....');
