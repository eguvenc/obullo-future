...
Candy.Core = (function(self, Strophe, $) {
	...
	var _connection = null,
		...
		_registerEventHandlers = function() {
			...
			self.addHandler(self.Event.Jabber.Roster, Strophe.NS.ROSTER, 'iq');
...
Candy.View = (function(self, $) {
	...
	self.init = function(container, options) {
		...
		_current.container.html(Mustache.to_html(Candy.View.Template.Chat.pane, {
            ...
            mroster : Candy.View.Template.Chat.mroster
		}));
...
Candy.Core.ChatRoom = function(roomJid) {
    ...
    this.mroster = new Candy.Core.MRoster();
     ...
     *
     * Parameters:
     *   (Candy.Core.MRoster) roster - Main roster
     */
    this.setMRoster = function(roster) {
     ...
     *
     * Returns
     *   (Candy.Core.MRoster) - instance
     */
    this.getMRoster = function() {
        return this.mroster;
    };
};
/** File: chatRoster.js
...
/** Class: Candy.Core.MRoster
 * Main Roster
 */
Candy.Core.MRoster = function () {
    /** Object: items
     * Roster items
     */
    this.items = {};
     ...
     *
     * Parameters:
     *   (Candy.Core.MRoster) user - User to add
     */
    this.add = function(user) {
        this.items[user.getJid()] = user;
    };
...
Candy.Core.Event = (function(self, Strophe, $, observable) {
	...
	self.Jabber = {
        ...
        */
        Roster: function(msg) {
            Candy.Core.log('[Jabber] MRoster');
            if (msg != '') Candy.Core.Event.Jabber._mrostermsg = msg;
            var t = $('#mroster-pane');
            t.html('');
            var me = Candy.Core.getUser().getJid();
            var roster = $(msg).children().children(); //safari fix
            var online = [];
            var offline = [];
			_online =  $.i18n._('online');
			_offline = $.i18n._('offline');
			
            
            $(roster).each(function(index, value) {
                value = $(value);
                var result = $.grep(self.Jabber.mroster, function(e){ return e.jid == value.attr('jid'); });
                if (result.length == 0) offline.push(value);
                else {
                    online.push({'mr':result[0],'value':value});
                }
            });
            var _s = '<div class="mroster_online" onclick="$(\'#mroster_online_list\').toggle();">'+_online+' ('+online.length+')</div><div id="mroster_online_list">';
            for(var i=0;i<online.length;i++) {
                var value = online[i].value;
                var user = new Candy.Core.ChatUser(value.attr('jid'), Strophe.getNodeFromJid(value.attr('jid')));
                var userId = Candy.Util.jidToId(user.getJid());
                _s += '<div onclick="Candy.View.Pane.MRoster.userClick(this);" data-affiliation="none" data-role="participant" data-nick="'+user.getNick()+'" data-jid="'+online[i].mr.privateroom+'" id="user--'+userId+'" class="user role-participant affiliation-none onlineuser" style="display: block; opacity: 1;"><div class="label">'+user.getNick()+ ((online[i].mr.status != '')?'&nbsp;&nbsp;&nbsp;-&nbsp;&nbsp;&nbsp;' + online[i].mr.status:'') +'</div><ul><li id="context--'+userId+'" class="context"></li></ul></div>';
            }
            _s +='</div><div class="mroster_offline" onclick="$(\'#mroster_offline_list\').toggle();">'+_offline+' ('+offline.length+')</div><div id="mroster_offline_list">';
            
            for(var i=0;i<offline.length;i++) {
                var value = offline[i];
                var user = new Candy.Core.ChatUser(value.attr('jid'), Strophe.getNodeFromJid(value.attr('jid')));
                var userId = Candy.Util.jidToId(user.getJid());
                _s += '<div onclick="Candy.View.Pane.MRoster.userClick(this);" data-affiliation="none" data-role="participant" data-nick="'+user.getNick()+'" data-jid="'+user.getJid()+'" id="user--'+userId+'" class="user role-participant affiliation-none" style="display: block; opacity: 1;"><div class="label">'+user.getNick()+'</div><ul><li id="context--'+userId+'" class="context"></li></ul></div>';
            }
            t.html(_s+'</div');
            return true;
        },
		...
		Presence: function(msg) {
            ...
            //get online
            var reparse = false;
            if(msg.attr('xmlns') == Strophe.NS.CLIENT) {
                if (msg.attr('type') === 'error') {
                    self.Jabber.Room.PresenceError(msg);
                } else {
                    if (typeof msg.attr('from') != 'undefined') {
                        var me = Candy.Core.getUser();
                        //type="unavailable" for the gone
                        if (Strophe.getBareJidFromJid(msg.attr('from')) != me.getJid()) {
                            var result = $.grep(self.Jabber.mroster, function(e){ return e.jid == Strophe.getBareJidFromJid(msg.attr('from')); });
                            if (result.length == 0) { //became online
                                var _mroster = {'jid':Strophe.getBareJidFromJid(msg.attr('from')),'privateroom':msg.attr('from'),'status':'','priority':'','to':msg.attr('to')};
                                if (msg.children('priority').length > 0) _mroster.priority = msg.children('priority')[0].innerHTML;
                                if (msg.children('status').length > 0) _mroster.status = msg.children('status')[0].innerHTML;
                                self.Jabber.mroster.push(_mroster);
                            } else {
                                if (msg.attr('type') == 'unavailable') { //became offline
                                    var nmroster = [];
                                    var tjid = Strophe.getBareJidFromJid(msg.attr('from'));
                                    for(var i=0;i<self.Jabber.mroster.length;i++)
                                        if (self.Jabber.mroster[i].jid != tjid) nmroster.push(self.Jabber.mroster[i]);
                                    self.Jabber.mroster = nmroster;
                                }
                            }
                            reparse = true;
                        }
            ...
            if (reparse) Candy.Core.Event.Jabber.Roster(Candy.Core.Event.Jabber._mrostermsg);
			return true;
		},
...
Candy.View.Event = (function(self, $) {
	...
	self.Room = {
		...
		onShow: function(args) {
            if (Candy.View.Pane.Room._groupchatrooms.length == 1) {
                $('#chat-tabs li a.close').hide();
            } else {
                $('#chat-tabs li a.close').show();
            }
			return;
		},
    ...
    /** Class: Candy.View.Event.MRoster
     * Roster-related events
     */
    self.MRoster = {
        /** Function: onUpdate
	...
	self.Presence = {
		...
		update: function(obj, args) {
			...
			// A user changed presence
                ...
                //if there's only one tab then show it..
                if (Candy.View.Pane.Room.getCount() == 1) {
                    Candy.View.Pane.Room.show(args.roomJid);
                }
			}
		},
	...
	self.Message = {
		...
		update: function(obj, args) {
			if(args.message.type === 'subject') {
                ...
                var rc = Candy.View.Pane.Room.getCount();
                //if there's only one tab then show it..
                if (rc == 1) {
                    Candy.View.Pane.Room.show(args.roomJid);
                } else if (rc > 1) {
                    //todo:show the first room
                }
			}
		}
	};
...
Candy.View.Pane = (function(self, $) {
	...
	self.Chat = {
		...
		tabClick: function(e) {
			...
			if (typeof self.Chat.rooms[currentRoomJid] != 'undefined')
		...
		allTabsClosed: function() {
			//below are commented out, as user can always get a private message from the main roster
	...
	self.Room = {
		...
		init: function(roomJid, roomName, roomType) {
            ...
            if (roomType == 'groupchat' && $.inArray(roomJid,self.Room._groupchatrooms)) {
                self.Room._groupchatrooms.push(roomJid);
            }
        ...
        getCount : function () {
            var roomJid;
            var count = 0;
            for(roomJid in self.Chat.rooms) if (self.Chat.rooms.hasOwnProperty(roomJid)) count++;
            return count;
        }
	};
	...
	self.PrivateRoom = {
         ...
         *   - <Candy.View.Event.Room.onAdd>
         */
        openMR: function(roomJid, roomName, switchToRoom, isNoConferenceRoomJid) {
            var user = new Candy.Core.ChatUser(roomJid,Strophe.getNodeFromJid(roomJid));
            var me = Candy.Core.getUser();
            // if target user is in privacy list, don't open the private chat.
            if (me.isInPrivacyList('ignore', roomJid)) {
                return false;
            }
            if(!self.Chat.rooms[roomJid]) {
                self.Room.init(roomJid, roomName, 'chat');
            ...
            self.Roster.update(roomJid, new Candy.Core.ChatUser(roomJid, roomName), 'join', me);
            self.Roster.update(roomJid, me, 'join', me);
            self.PrivateRoom.setStatus(roomJid, 'join');
    ...
    self.MRoster = {
         ...
         *   (Candy.Core.ChatUser) currentUser - Current user
         */
        update: function(user, currentUser) {
            var userId = Candy.Util.jidToId(user.getJid()),
                usercountDiff = -1;
                
            var html = Mustache.to_html(Candy.View.Template.MRoster.user, {
                    userId : userId,
                    userJid: user.getJid(),
                    nick: user.getNick(),
                    displayNick: Candy.Util.crop(user.getNick(), Candy.View.getOptions().crop.roster.nickname),
                ...
                userElem = $('#mrosteruser-' + userId);

            if(userElem.length < 1) {
                var userInserted = false,
                    rosterPane = self.Room.getPane(roomJid, '.mroster-pane');
                // there are already users in the roster
                if(rosterPane.children().length > 0) {
                    // insert alphabetically
                    var userSortCompare = user.getNick().toUpperCase();
            ...
            Candy.View.Event.MRoster.onUpdate({'user' : user, 'element': $('#mrosteruser-' + userId)});
        },

        /** Function: userClick
         * Click handler for opening a private room
         */
        userClick: function(o) {
            var elem = $(o);
            self.PrivateRoom.openMR(elem.attr('data-jid'), elem.attr('data-nick'), true, false);
        },
...
Candy.View.Template = (function(self){
	...
	self.Chat = {
		pane: '<div id="chat-pane">{{> tabs}}{{> toolbar}}{{> rooms}}{{> mroster}}</div>{{> modal}}',
        ...
        mroster : '<div id="mroster-pane" class="mroster-pane"></div>',
    ...
    /*self.MRoster = {
        pane: '<div class="mroster-pane" id="mroster-pane0"></div>',
        user: '<div class="user role-{{role}} affiliation-{{affiliation}}{{#me}} me{{/me}}" id="user-{{roomId}}-{{userId}}" data-jid="{{userJid}}" data-nick="{{nick}}" data-role="{{role}}" data-affiliation="{{affiliation}}"><div class="label">{{displayNick}}</div><ul><li class="context" id="context-{{roomId}}-{{userId}}"></li><li class="role role-{{role}} affiliation-{{affiliation}}" data-tooltip="{{tooltipRole}}"></li><li class="ignore" data-tooltip="{{tooltipIgnored}}"></li></ul></div>'
    };*/
...
Candy.View.Translation = {
	...
	'tr' : {
		...
		'statusDisconnecting': 'Bağlantı Kopuyor...',
		'statusDisconnected' : 'Bağlantı Koptu',
		'statusAuthfail': 'Geçersiz Giriş Bilgileri',
		...
		'labelPassword': 'Şifre:',
		'loginSubmit'  : 'Giriş',
		'loginInvalid'  : 'Geçersiz JID',

		'reason'				: 'Neden:',
		'subject'				: 'Konu:',
		'reasonWas'				: 'Reason was: %s.',
		'kickActionLabel'		: 'Engelle',
		'youHaveBeenKickedBy'   : 'You have been kicked from %2$s by %1$s',
		'youHaveBeenKicked'     : 'You have been kicked from %s',
		'banActionLabel'		: 'Ban',
		'youHaveBeenBannedBy'   : 'You have been banned from %1$s by %2$s',
		...
		'privateActionLabel' : 'Özel Chat',
		'ignoreActionLabel'  : 'Engelle',
		'unignoreActionLabel' : 'Engeli Kaldır',

		'setSubjectActionLabel': 'Konuyu Değiştir',

		'administratorMessageSubject' : 'Yönetici',
		...
		'tooltipRole'			: 'Moderator',
		'tooltipIgnored'		: 'You ignore this user',
		'tooltipEmoticons'		: 'İfadeler',
		'tooltipSound'			: 'Tüm mesajlarda sesli bildir',
		'tooltipAutoscroll'		: 'Otomatik Kaydır',
		'tooltipStatusmessage'	: 'Durum mesajlarını görüntüle',
		'tooltipAdministration'	: 'Room Administration',
		'tooltipUsercount'		: 'Room Occupants',
