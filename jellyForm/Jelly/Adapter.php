...
Class Jelly_Adapter
{
    ...
    {
        $this->c = $c;
        $this->view   = new Jelly_View_View($this->c);
        $this->rbac   = $this->c['rbac'];
    ...
    /**
     * Set col-sm for bootstrap template
     * 
     ...
     * @return void
     */
    public function setColSm($field, $sm)
    {
        $this->colSm[$field] = $sm;
    }
    ...
    {
        $this->isRules = true;
        $this->c['validator']->setRules($field, $label, $rules);
    }
     ...
     * @return void
     */
    public function addAfterGroup($groupId, $data)
    {
        $this->afterGroup[$groupId] = $data;
    }

    /**
     * Append group special value
     * 
     ...
     * @return void
     */
    public function appendGroup($order, $data)
    {
        $this->appendGroup[$order] = $data;
    }
    ...
    public function getPostValues()
    {
        ...
        foreach ($formElements as $val) {
            ...
            if ($request->post($val[static::ELEMENT_NAME]) !== false) {
                $formValues[$val[static::ELEMENT_NAME]] = $request->post($val[static::ELEMENT_NAME]);
    ...
    //  * @return void
    //  */
    // public function appendGroup($order, $data = array())
    // {
    //     foreach ($this->getDefaultAppendAttributes() as $key => $val) {
    //         if (isset($data[$key])) {
    //             $this->data['appendGroup'][$data['name']][$val] = $data[$key];
    //         } else {
    //             $this->data['appendGroup'][$data['name']][$val] = '';
    //         }
    //     }
    //     $this->data['appendGroup'][$data['name']][static::ELEMENT_ORDER]    = (int)$order;
    //     // $this->data['appendGroup'][$data['name']][static::ELEMENT_GROUP_ID] = (isset($data[static::ELEMENT_GROUP_ID])) ? $data[static::ELEMENT_GROUP_ID] : 0;
    // }
    ...
    public function increaseAppendOrder($elementName, $order)
    {
        ...
        // if (isset($this->data['appendGroup']) AND ! isset($this->data['appendGroup'][$elementName])) {
        //     foreach ($this->data['appendGroup'] as $v) {
        //         if ($order >= $v[static::ELEMENT_ORDER]) {
        //             $order++;
        //         }
        //     }
    ...
    public function renderArray($formData, $elementData)
    {
        ...
        // if (isset($this->data['appendGroup'])) {
        //     foreach ($this->data['appendGroup'] as $value) {
        //         array_push($elementData, $value);
        //     }
        // }
    ...
    public function buildArray($data)
    {
        ...
        $this->rbac->resource->object[$formData[static::FORM_NAME]]->getPermissions(['view', $this->operationName]);
        $formPermission = $this->rbac->user->view->object[$formData[static::FORM_NAME]]->isAllowed();
        $formOpPermission = $this->rbac->user->{$this->operationName}->object[$formData[static::FORM_NAME]]->isAllowed();

        // $formOpPermission   = $this->rbac->hasObjectPermission($formData[static::FORM_NAME], $this->operationName);
        // $formPermission     = $this->rbac->isPermitted($formData[static::FORM_NAME], $formViewPermission);
        ...
        if ($formOpPermission === false) {
            // $elementViewPermissions = $this->rbac->hasChildPermission($formData[static::FORM_NAME], $this->getFormNames(static::ELEMENT_NAME, $elementData), 'view');
            // $elementOpPermissions   = $this->rbac->hasChildPermission($formData[static::FORM_NAME], $this->getFormNames(static::ELEMENT_NAME, $elementData), $this->operationName);
            // $groupOpPermissions = (count($groupData) > 0) ? $this->rbac->hasChildPermission($formData[static::FORM_NAME], $this->getFormNames(static::GROUP_NAME, $groupData), $this->operationName) : false;
            
            // $this->rbac->resource->object[$formData[static::FORM_NAME]]
            //     ->getPermissions(
            //         $this->getFormNames(static::ELEMENT_NAME, $elementData),
            //         'view'
            //     );
            // $elementViewPermissions = $this->rbac->view->object[$formData[static::FORM_NAME]]->isAllowed();

            $this->rbac->resource->object[$formData[static::FORM_NAME]]
                ->getPermissions(
                    $this->getFormNames(static::ELEMENT_NAME, $elementData),
                    ['view', $this->operationName]
                );

            // $this->rbac->resource->object[$formData[static::FORM_NAME]]
            //     ->getPermissions(
            //         $this->getFormNames(static::GROUP_NAME, $groupData),
            //         $this->operationName
            //     );
            // $groupOpPermissions = $this->rbac->{$this->operationName}->object[$formData[static::FORM_NAME]]->isAllowed();
        ...
        foreach ($elementData as $val) {
            ...
            if ($formPermission === true) {
                ...
                if ($formOpPermission === false
                    && $role !== 'widget'
                    && $val[static::ELEMENT_TYPE] !== 'submit'
                    && $this->rbac->user->{$this->operationName}->object[$formData[static::FORM_NAME]]->isAllowed($val[static::ELEMENT_NAME]) === false
                ) {
            ...
            } elseif (isset($this->data['append'][$val[static::ELEMENT_NAME]])
                || $this->rbac->user->view->object[$formData[static::FORM_NAME]]->isAllowed($val[static::ELEMENT_NAME]) === true
            ) {
            
                // If the user doesn't have any permissions, we are add "disabled" the input attribute.
                if ($this->rbac->user->{$this->operationName}->object[$formData[static::FORM_NAME]]->isAllowed($val[static::ELEMENT_NAME]) === false
                    && $role !== 'widget'
                    && $val[static::ELEMENT_TYPE] !== 'submit'
                ) {
                    if (! isset($this->data['append'][$val[static::ELEMENT_NAME]])) {
        ...
        if (isset($submitTempOrder)) {
            ...
            if ($formOpPermission === false AND $submitIsOpen === false) {
    ...
    {
        // $request = $this->c['return request'];
        $this->validator = $this->c['return validator'];
        ...
        // $formOpPermission   = $this->rbac->hasObjectPermission($formData[static::FORM_NAME], $operationName);   // Get form operation permission.
        // $formPermission     = $this->rbac->isPermitted($formData[static::FORM_NAME], $formOpPermission);        // Check form permission.
        // $elementsPermission = $this->rbac->hasObjectPermission(array_keys($newValues), $operationName);         // Elements permission.
        
        // $formPermission = $this->rbac->user->view->object[$formData[static::FORM_NAME]]->isAllowed();
        $this->rbac->resource->object[$formData[static::FORM_NAME]]->getPermissions(['view', $operationName]);
        $formPermission = $this->rbac->user->{$operationName}->object[$formData[static::FORM_NAME]]->isAllowed();

        $this->rbac->resource->object[$formData[static::FORM_NAME]]->getPermissions(array_keys($newValues), $operationName);         // Elements permission.
        ...
        if ($formPermission === false) {
            ...
            foreach ($newValues as $key => $val) {
                ...
                if ($this->rbac->user->{$operationName}->object[$formData[static::FORM_NAME]]->isAllowed($key) === false) {
    ...
    {
        $className = '\\Jelly_Widget_'. ucfirst($class);
    ...
    public function load($class, $formId = '')
    {
        ...
        $className              = '\\Jelly_Html_Form_'. ucfirst($class);
